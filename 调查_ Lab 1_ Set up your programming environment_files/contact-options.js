eesy.define(['jquery-private', 'json!language-supportcenter', 'json!settings-supportcenter', 'mustachejs'],
  ...
  function childrenByType(type, nodeId, onResponse) {
    return $.get(dashboardUrl + '/rest/nodes/' + nodeId
        + '/children?type=' + type + '&sessionkey=' + sessionKey, onResponse);
  }
  ...
  function readContactOptions(layoutNodeId, onDone) {
    childrenByType('contactoption', layoutNodeId, function(_contactOptions) {
      ...
      $.each(contactOptions, function(i, o) {
        ...
        requests.push(childrenByType('enabled_weekly', o.id, function(enabledWeeklies) {
          o.enabledWeeklies = enabledWeeklies;
        }));

        requests.push(childrenByType('disabled_period', o.id, function(disabledPeriods) {
  ...
  function initForNodeId(nodeId, onNodeInited) {
    $.get(dashboardUrl + '/rest/contactoptions/' + nodeId
        + '/enabledContactOptions?sessionkey=' + sessionKey, function(enabledIds) {
  ...
  function isOpen(contactOption) {
    ...
    $.each(contactOption.enabledWeeklies, function(i, w) {
      if (w.properties.day == dayOfWeekBase1
    ...
    $.each(contactOption.disabledPeriods, function(i, p) {
      if (true
          && now >= parseStamp(p.properties.from)
          && now <= parseStamp(p.properties.to)) {
  ...
  function getOpeningHoursText(contactOption) {
    ...
    if (isEmpty(contactOption.enabledWeeklies)) {
      ...
      text += $.map(contactOption.enabledWeeklies, function(w) {
    ...
    if (!isEmpty(contactOption.disabledPeriods)) {
      ...
      text += $.map(contactOption.disabledPeriods, function(p) {

eesy.define(['jquery-private', 'osage-contact', 'osage-selector', 'system-information', 'utils',
        'field-decorator',
        'json!language-supportcenter', 'json!settings-supportcenter', 'session-events'],
    function($, osageContact, osageSelector, systemInformation, utils,
             fieldDecorator,
             language, settings, sessionEvents) {
        ...
        function setupContactTemplate(channel, template, recipientOptions) {
            ...
            $.get(dashboardUrl + '/rest/contactform/' + nodeId + '/' + channel + '/fields?sessionkey='+sessionKey, function(_fields) {
                var hideTimeSelector = _fields.some(function(item) { return item.useTimeOfDay === 'false'; });
                decoratedFields = fieldDecorator.decorateFields(_fields, categoryPath());

                $('.eesy__modal__box main').html(template.html({
                    fieldsHtml: templates.contact_fields.html({ fields: decoratedFields, hideTimeSelector: hideTimeSelector }),
                    recipientOptions: recipientOptions,
                    hideRecipientSelector: hideRecipientSelector,
                }));

                $('.eesy__modal-scope .form__item input,.eesy__modal-scope .form__item textarea,.eesy__modal-scope .form__item select').each(function(){
                ...
                if (!hideRecipientSelector) {
                    ...
                    if (recipientOptions.length > 1) {
                        $('.chat-selector#dropdown-recipient-select[data-selector] '
                            + 'option[value="' + recipientOptions[0].id + '"]').attr('selected', 'selected');
                    }

                    osageSelector.decorate('#dropdown-recipient-select[data-selector]');

                    // handle frame change (only applies to chat)
                    utils.replaceLiveHandler('change', '.eesy__modal-scope .chat-selector#dropdown-recipient-select[data-selector]', function() {
                        $.each(recipientOptions, (function(selected) { return function(i, o) {
                    ...
                    utils.replaceLiveHandler('change', '.eesy__modal-scope .__static-call #dropdown-recipient-select[data-selector]', function() {
        ...
        function categoryPath() {
            return $.map(activeOptionPath(), function(node) {
        ...
        function validateForm(channel) {
            ...
            if (contactOptions.length != 1 && $('select#dropdown-recipient-select').val() == null) {
                $('.eesy__modal-scope select#dropdown-recipient-select')
                    .parent()
                    .parent()
                    .append('<div class="form__error isFocusable focusStyleUnderline" style="display: block;">' + language.LNG.SUPPORT_CENTER.MAIL.FORM.RECIPIENT_INVALID + '</div>');
            ...
            $.each(decoratedFields, function(i, field) {
                if (field.validator && !field.validator(field.id)) {
                    const errorMessage = field.validationFailedText ?? language.LNG.SUPPORT_CENTER.FORM.DEFAULT_VALIDATION_ERROR
                    const errorHtml = `
        ...
        function submitForm(channel, onFormSent) {
            ...
            $.get(dashboardUrl + '/rest/user/?sessionkey=' + sessionKey, function(user) {
                $.getJSON(dashboardUrl + '/rest/user/courses/names?sessionkey=' + sessionKey, function(courseNames) {
                    ...
                    if (hasCourse()) {
                        $.getJSON(dashboardUrl + '/rest/user/courses/' + (!(typeof eesy_course_id === 'undefined') ? eesy_course_id : -1), function(activeCourse) {
                            populateAndSendMail(channel, user, subject, courseNames, onFormSent, activeCourse.course_id ? activeCourse : undefined);
                        });
                    } else {
        ...
        function populateAndSendMail(channel, user, subject, courseNames, onFormSent, activeCourse) {
            ...
            const toContactOptionNodeId = contactOptions.length === 1 ? contactOptions[0].id : $('select#dropdown-recipient-select').val();

            if ((!settings.SUPPORTCENTER.PHONE.USE_STATIC_NUMBER && channel === 'phone') || channel === 'email') {
                $.post(dashboardUrl + "/rest/contactoptions/" + toContactOptionNodeId + '?sessionkey=' + sessionKey, {
                    mailHtml: mailHtml,
        ...
        $(function() {
            ...
            utils.replaceLiveHandler('click', '.eesy__modal-scope .form__submit.___call', function() {
                ...
                submitForm('phone', function(toContactOptionNodeId) {
                    ...
                    $.get(dashboardUrl + '/rest/nodes/' + toContactOptionNodeId + '?sessionkey=' + sessionKey, function(contactNode) {

eesy.define(['jquery-private', 'mustachejs', 'osage-selector',
        'contact-options', 'templates', 'contact-options-handler',
        'utils', 'sessionInfo', 'json!language-supportcenter', 'session-events',
        'helpitem-visibility', 'json!context-node-link-data',
        'json!settings-supportcenter', 'json!settings-uef'],
    function($, Mustache, osageSelector,
             contactOptionsModule, templates, contactOptionsHandler,
             utils, sessionInfo, language, sessionEvents, helpitemVisibility, contextNodeLinks,
             settings, settingsUef) {
        ...
        function selectNode(nodeId, onNodeSelected) {
            ...
            $.get(sessionInfo.dashboardUrl() + '/rest/nodes/' + activeNodeId + '/path?sessionkey=' + sessionInfo.sessionKey(),
                function(nodePath) {
        ...
        function getChildOptions(nodeId, fnChildOptions) {
            $.get(
                sessionInfo.dashboardUrl() + '/rest/nodes/' + nodeId + '/children?type=option,link&sessionkey=' + sessionInfo.sessionKey(),
                fnChildOptions);
        }

        function autoSelectNode(node) {
            if (contextNodePath.length == 0)
                return node.properties.auto_select == "true";
            ...
            if (itemToDelete)
                contextNodePath.splice( $.inArray(itemToDelete, contextNodePath), 1 );
        ...
        function showSearchResults(searchString) {
            if (searchString.length) {
                loadAndPresentHelpItems(
                    sessionInfo.dashboardUrl() + '/rest/helpitems/search?sessionkey=' + sessionInfo.sessionKey() + '&languageId=' + var_language,
                    { s: searchString },
        ...
        function loadHelpItemSuggestions(nodeId, onDone) {
            $.get(sessionInfo.dashboardUrl() + '/rest/nodes/' + nodeId + '?sessionkey=' + sessionInfo.sessionKey(), function(node) {

                $.get(sessionInfo.dashboardUrl() + '/rest/nodes/' + nodeId + '/children?type=helpsuggestion&sessionkey=' + sessionInfo.sessionKey(),
                    function(suggestions) {

                        var helpitemsUrl = sessionInfo.dashboardUrl() + '/rest/public/helpitems?sessionkey=' + sessionInfo.sessionKey()
                        ...
                        loadAndPresentHelpItems(
                            ...
                            false,
                            e => {
                                e.source = (node.properties.context_sensitive_help == "true" && contextHelpItems.includes(e.id))
        ...
        function loadAndPresentHelpItems(url, data, title, noResultTitle, contactOptions, showFilters, addSourceInformation, onDone) {
            $.get(url, data, function(helpitems) {
                ...
                if (!!sessionStorage.getItem("eesy_uef_support_center_loaded") && !!sessionStorage.getItem("eesy_UEFDefaultHelp")) {
                    ...
                    var defaultBBHelp = "" +
                        ...
                        "  <div class='results__subtitle isFocusable focusStyleUnderline' tabindex='0'>" + helpName + "</div>" +
                        "</li>";
        ...
        function displayResult(showFilters) {
            ...
            $contact.html(templates.contact_options.html({
        ...
        function showHelpItem(helpitem, contactOptions) {
            ...
            $('.eesy__modal__box main').html(templates.content_main.html({
                helpitem: helpitem,
            ...
            if ($("#eesy_embed_content")[0]) {
                ...
                var item_url = $( "div.eesy_url" ).text().replace(/ /g, '').replace('http://', '').replace('https://', '');
                ...
                $('#eesy_embed_content').load(var_dashboard_url + '/rest/content/partialHtmlPage?url=//' + item_url + '&selector=body' + '&deleteSelector=' + deleteSelectors + ' ' + selectors, function(response, status, xhr) {
                    if (status == "error") {
        ...
        function loadAndShowHelpitem(helpitemId) {
            ...
            $.get(sessionInfo.dashboardUrl() + '/rest/public/helpitems/' + helpitemId + '/?sessionkey='
                + sessionInfo.sessionKey() + '&languageId=' + var_language,
                function(helpitem) {
        ...
        function installGuiHandlers() {
            ...
            function helpitemInListClicked(helpItemListItem) {
                ...
                if (hid !== undefined) {
                    sessionEvents.addHelpitemOpenedInContexts($.map(ruleIds, function(ruleId) {
                        return {
                            helpItemId: hid,
                            contextId: ruleId,
            ...
            function selectButtonHandler(element) {
                ...
                // Remove nodes below
                $(element).parents('.eesy-node').nextAll().slideUp(SLIDING_DURATION_MS, function() {
                    $(this).remove();
                });

                if (settings.SUPPORTCENTER.WCAG.ENABLED) {
                    if (!buttonRemoved) {
                        $(element).append("<button class='xbutton__close' id='xbutton' role='button' aria-label='exit' label='Exit'>X</button>");
                    }
                }
                ...
                if (isDeselecting) {
                    ...
                    if (collapse) {
                        ...
                        if (!settings.SUPPORTCENTER.WCAG.ENABLED) {
                            $(element).append("<div class='xbutton__close' id='xbutton' role='button' aria-label='exit' label='Exit'></div>");
                        }
            ...
            utils.replaceLiveHandler('change', '.eesy__modal-scope select.optionDropdown[data-selector]', function() {
                $(this).parents('.eesy-node').nextAll().slideUp(SLIDING_DURATION_MS, function() {
                    $(this).remove();
                });
            ...
            function handleContactButton(selector, channel, templateName) {
                utils.onClickOrSelectKey(selector, function(e) {
                    ...
                    sessionEvents.addAssistedSupportRequestInitiated(channel, activeNodePath[activeNodePath.length - 1].id);
                    ...
                    if (channel == 'phone') {
                        utils.focusElement(settings.SUPPORTCENTER.WCAG.ENABLED ? "#keepRefText" : "#submitPhoneForm", 500);
                    }
            ...
            utils.replaceLiveHandler('input', `${CONTACT_FORM_SELECTOR} input, ${CONTACT_FORM_SELECTOR} textarea`, onContactFormDataChange);
        ...
        function positionUpButton() {
            if (!var_uefMode && !var_uefModeOriginalUseUefSupportCenter) { // none ultra style
        ...
        function getPreferredLayout(layouts) {
            ...
            $.each(layouts, function (idx, layoutCandidate) {
                if(layoutCandidate.properties.prefered_when_in_mode && layoutCandidate.properties.prefered_when_in_mode === sessionStorage.getItem("lmsMode")) {
        ...
        async function getLayout() {
            ...
            return $.get(sessionInfo.dashboardUrl() + '/rest/nodes?type=dashboard_layout&sessionkey=' + sessionInfo.sessionKey()).then((nodes) => {
        ...
        function _show(_contextHelpItems, _contextNodes, hid, _autoActivateHelpitemId, _ruleIds) {
            eesyRequire(['json!user-context-variables'], function(response) {
                ...
                var helpItem = sessionStorage.getItem("helpItemArticleHandle") !== null
                    ? JSON.parse(sessionStorage.getItem("helpItemArticleHandle"))
                    : null;
                var helpItemId = sessionStorage.getItem("uefSupportCenterArticleId");

                if (var_uefMode && (helpItem || helpItemId)) { // handle helpItem linking for bb ultra
                    if (helpItemId) {
                    ...
                    contextHelpItems = _contextHelpItems === "" || _contextHelpItems == null ? [] : _contextHelpItems.split(",");
                    ...
                    if (contextNodes.length > 0) {
                        requests.push($.get(sessionInfo.dashboardUrl() + '/rest/nodes/' + getContextNodeIdThatShouldBeActivated(contextNodes) + '/path?sessionkey=' + sessionInfo.sessionKey(), function (v) {
                            contextNodePath = v;
                        }));
                    }
                    ...
                    function initSupportCenter() {
                        renderSupportCenter().then(async () => {
                            ...
                            contactOptionsHandler.setup(templates, contactOptionsModule, sessionInfo.sessionKey(), sessionInfo.dashboardUrl());
                            const layout = await getLayout();

                            maxInitialHelpitems = layout.properties.number_of_initial_visible_helpitems == undefined
                                ? undefined
                                : parseInt(layout.properties.number_of_initial_visible_helpitems);

                            contactOptionsModule.init(sessionInfo.dashboardUrl(), sessionInfo.sessionKey(), supportCenterId, function () {
        ...
        function themeCssUrl(layout) {
            return sessionInfo.dashboardUrl("/rest/public/support-center/" + layout.id + "/theme.css" +
        ...
        function uefOriginalSupportCenterStyling() {
            ...
            $eesyModalScope.prepend('<a role="button" id="panelType-full" title="Resize panel">' +
                ...
                '</svg></a>');
            $eesyModalScope.prepend('<a role="button" id="panelType-small" title="Resize panel">' +
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40" fill="none">' +
            ...
            $uefOriginalHeader.append('<a class="header__close" tabindex="0" role="button" aria-label="close"><div id="close-uef-svg"></div></a>');
        ...
        function resizeUefFooter(panelType) {
            $('.eesy-uef-original-override .eesy__modal-scope .eesy__modal .layout__footer')
                .css("width", panelType === "full" ? "95%" : "420px").css("transition", "none");
        }
        ...
        function resizeUefPanel(panelType) {
            ...
            $('.eesy__modal-scope')
                .css("width", isFullPanel ? "95%" : "420px").css("transition", "width 0.5s");
            $('.eesy__modal__background')
                .css("right", isFullPanel ? "95%" : "420px").css("transition", "right 0.5s");
            $('.eesy-uef-original-override .eesy__modal-scope .eesy__modal .layout__footer')
                .css("width", isFullPanel ? "95%" : "420px").css("transition", "width 0.5s");
        }
        ...
        function showHelpItemInNode(helpItemGuid) {
            $.get(sessionInfo.dashboardUrl() + "/rest/public/helpitemid?guid=" + helpItemGuid, function(response) {
                ...
                $.get(sessionInfo.dashboardUrl() + "/rest/public/helpitemSupportCenterNode?guid=" + helpItemGuid, function(response) {

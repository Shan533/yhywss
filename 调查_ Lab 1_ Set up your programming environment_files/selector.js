eesy.define(['jquery-private', 'json!settings-supportcenter', 'utils'], function($, settings, utils) {
    ...
    function selectOption(el, close) {
        ...
        if ($selectedOption.attr('data-value')) {
            ...
            $selectedOption.closest('.selector').find('[value="'+ selectedValue +'"]').prop('selected', true).change();
        ...
        decorate : function(jquerySelector) {
            ...
            function initSelector() {
                ...
                function buildSelector() {
                    ...
                    $dataSelector.each(function(index) {
                        ...
                        if (!settings.SUPPORTCENTER.WCAG.ENABLED) {                        
                            ...
                            $(this).parent().append('<div class="selector__select isFocusable focusStyleOutline" tabindex="'+ tabIndex + '"></div><ul class="selector__options"></ul>');
                ...
                $selector.off('keydown').on('keydown', function(ev) {
                    ...
                    if (utils.isSelectKey(ev)) { // select keys
                        ev.preventDefault();
                        ...
                        if (ev.keyCode === 38) { // up
                            ...
                            if (!prev.length) { // is first item in list
                            ...
                            if (!found.length) {
                                ...
                                if (!next.length) { // is last item in list
                                    next = $(this).find('.selector__option:not(.___is-disabled)').first();
                                }
                            }
